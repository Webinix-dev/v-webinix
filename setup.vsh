...
// https://api.github.com/repos/webinix-dev/webinix/releases
struct Release {
	tag_name   string
	prerelease bool
}
...
const webinix_release_base_url = 'https://github.com/webinix-dev/webinix/releases'
const platform = pref.get_host_os()
const arch = pref.get_host_arch()
const archives = {
	pref.OS.linux: {
		pref.Arch.amd64: 'webinix-linux-gcc-x64.zip'
		.arm64:          'webinix-linux-gcc-arm64.zip'
		.arm32:          'webinix-linux-gcc-arm.zip'
	}
	.macos:        {
		pref.Arch.amd64: 'webinix-macos-clang-x64.zip'
		.arm64:          'webinix-macos-clang-arm64.zip'
	}
	.windows:      {
		pref.Arch.amd64: 'webinix-windows-gcc-x64.zip'
	}
}

fn get_webinix_releases() !Releases {
	mut request := http.Request{}
	token := os.getenv('WEBUI_GH_TOKEN')
	if token != '' {
		request.header.add(.authorization, 'Bearer ${token}')
	}
	request.url = 'https://api.github.com/repos/webinix-dev/webinix/releases'
	c_repo_resp := request.do()!
	if c_repo_resp.status_code != 200 {
		return error('Failed get success status when fetching Webinix releases: ${c_repo_resp}')
	}
	request.url = 'https://api.github.com/repos/webinix-dev/v-webinix/releases'
	v_repo_resp := request.do()!
	if v_repo_resp.status_code != 200 {
		return error('Failed get success status when fetching V-Webinix releases: ${v_repo_resp}')
	}
...
fn run(cmd cli.Command) ! {
	...
	releases := get_webinix_releases() or { return error('Failed to fetch releases: ${err}') }
	$if debug && verbose ? {
		dump(releases)
	}
	...
	if os.exists(out_dir) {
		input := os.input('Webinix output directory `${out_dir}` already exists.
${colorize(term.blue,
	...
	version := match version_arg {
		...
		'latest' {
			...
			}) or { return error('Failed to find Webinix release') }.tag_name
			// vfmt on
		}
		else {
			releases.find_matching_parent_tag(version_arg) or {
				return error('Failed to find the matching Webinix C release.')
			}
		}
	}

	tmp_dir := os.join_path(os.temp_dir(), 'webinix_${version}')
	os.mkdir(tmp_dir) or {}
	defer {
		rmdir_all(tmp_dir) or {}
	}

	println('Downloading Webinix@${version}...')
	url := '${webinix_release_base_url}/download/${version}/${archive}'
...
mut cmd := cli.Command{
	...
	flags: [
		cli.Flag{
			...
			name: 'output'
			abbrev: 'o'
			description: 'Specify the output path for the download Webinix platfrom release.'
			global: true
			default_value: [join_path(@VMODROOT, 'webinix')]
		},
	]
	execute: run
}
