...
fn test_get_js_arg() {
	...
	setTimeout(async () => {
		...
		await webinix.call("v_fn_with_string_arg", "foo");
		await webinix.call("v_fn_with_int_arg", 123);
		await webinix.call("v_fn_with_bool_arg", true);
		await webinix.call("v_fn_with_obj_arg", JSON.stringify(person));
		await webinix.call("v_fn_with_arr_arg", JSON.stringify([1, 2, 3]));
		await webinix.call("assert_and_exit")
	}, 500)'
...
fn test_get_js_arg_at_idx() {
	...
	setTimeout(async () => {
		const str = "foo"
			...
			array = [1, 2, 3];
		await webinix.call("assert_get_args_at_idx", str, int, bool, JSON.stringify(struct), JSON.stringify(array));
	}, 500)'
...
fn test_return_value_to_js() {
	...
	script := '
	setTimeout(async () => {
		const str = await webinix.call("v_return_string"),
			int = await webinix.call("v_return_int"),
			bool = await webinix.call("v_return_bool"),
			struct = await webinix.call("v_return_struct"),
			array = await webinix.call("v_return_array");
			console.log(struct)
		await webinix.call("assert_js_res_from_v", str, int, bool, struct, array);
	}, 500)'
	w.show(utils.gen_html(@FN, script),
		await: true
	) or { assert false, err.str() }
